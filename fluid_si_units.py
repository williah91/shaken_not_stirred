...
class OceanBloodSimulation:
    def __init__(self, grid_size=100, 
                 domain_size=20, # in meters
                 diffusion_coef=0.002, # in m^2/s
                 viscosity=0.02, # in Pa·s (kg/(m·s))
                 dt=0.1, # in seconds
                 pressure_iterations=75,
                 mode='stirring',             # 'stirring', 'shaking', 'random_eddies', 'combined_shake'
                 stir_strength=1.5, 
                 shake_accel_amplitude=50.0, # in m/s²
                 shake_frequency=2.0, # in Hz
                 random_amplitude=1.0,       # in m/s
                 random_correlation_sigma=8.0): # in grid cells

        self.grid_size = grid_size
        self.domain_size = domain_size # in meters
        self.dx = domain_size / grid_size # grid cell size in meters
        self.dy = domain_size / grid_size # grid cell size in meters
        self.dt = dt # time step in seconds
        self.diffusion_coef = diffusion_coef # in m²/s
        self.viscosity = viscosity # in Pa·s (kg/(m·s))
        self.pressure_iterations = pressure_iterations
        self.time = 0 # simulation time in seconds
        self.x = np.linspace(-domain_size/2, domain_size/2, grid_size) # x-coordinates in meters
        self.y = np.linspace(-domain_size/2, domain_size/2, grid_size) # y-coordinates in meters
        self.X, self.Y = np.meshgrid(self.x, self.y) # 2D grid of coordinates
        self.u_vel = np.zeros((grid_size, grid_size)) # x-velocity in m/s
        self.v_vel = np.zeros((grid_size, grid_size)) # y-velocity in m/s
        self.concentration = np.zeros((grid_size, grid_size)) # concentration in arbitrary units

        # mode and parameters
        self.mode = mode
        self.stir_strength = stir_strength # vortex strength in m/s
        self.shake_accel_amplitude = shake_accel_amplitude # in m/s²
        self.shake_frequency = shake_frequency # in Hz
        self.random_amplitude = random_amplitude # in m/s
        self.random_correlation_sigma = random_correlation_sigma # in grid cells
        
        # mass tracker for conservation
        self.initial_mass = 0.0 # initial mass in arbitrary units

        print(f"Simulation initialized in '{self.mode}' mode.")
        # ... (print parameters for the selected mode)

    def apply_vortex(self, strength): 
        # Applies a vortex force to the velocity field
        u_vortex_temp = np.zeros((self.grid_size, self.grid_size))
        v_vortex_temp = np.zeros((self.grid_size, self.grid_size))
        if self.mode != 'stirring': return u_vortex_temp, v_vortex_temp
        if self.time < self.total_runtime/3: current_strength = strength
        ...
        if current_strength > 1e-9:
            center_x, center_y = 0, 0 # vortex center at (0, 0) in meters
            dX = (self.X - center_x)*self.dx
            dY = (self.Y - center_y)*self.dy
            r = np.sqrt(dX**2 + dY**2) # distance to vortex center in meters
            mask = r < 0.45*self.dx # vortex radius in meters
            factor = np.exp(-r[mask]/4) # exponential decay factor
            u_vortex_temp[mask] = -current_strength * dY[mask] / r[mask] * factor * self.dx/self.domain_size
            v_vortex_temp[mask] = current_strength * dX[mask] / r[mask] * factor * self.dy/self.domain_size
        return u_vortex_temp, v_vortex_temp

    def source(self, x_pos=0, y_pos=0, volumeOfBlob=15.0, std=5):
        # Adds a Gaussian source of concentration to the grid
        ...
        distancesq = (y_idx-i)**2 + (x_idx-j)**2
        std_grid = std / self.dx if self.dx > 0 else std # standard deviation in grid cells
        self.concentration += volumeOfBlob * np.exp(-distancesq / (2 * max(std_grid, 1e-6)**2))
        
        # Calculate the initial total mass
        self.initial_mass = np.sum(self.concentration) * self.dx * self.dy # in arbitrary units
    ...
    def apply_boundary_conditions(self, u, v):
        # Applies no-slip boundary conditions (velocity = 0 at boundaries)
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                ...
                if np.sqrt(x**2 + y**2) >= 0.45*self.domain_size: # radius compared to 
                    u[i, j] = 0
                    v[i, j] = 0
    ...
    def _calculate_gradient(self, p):
        ...
        grad_p_y[0,:]=grad_p_y[1,:]; grad_p_y[-1,:]=grad_p_y[-2,:]; grad_p_y[:,0]=grad_p_y[:,1]; grad_p_y[:,-1]=grad_p_y[:,-2]; 
    ...
    def project_velocity(self, u, v):
        ...
        u_final, v_final = self.apply_boundary_conditions(u_corrected, v_corrected)
        
        # Normalize velocity to prevent excessive growth
        max_velocity = np.max(np.sqrt(u_final**2 + v_final**2))
        if max_velocity > 1e3:  # Threshold to prevent runaway velocities
            scaling_factor = 1e3 / max_velocity
            u_final *= scaling_factor
            v_final *= scaling_factor
    ...
    def simulate_step(self):
        ...
        u_forced += u_rand / self.dx
        v_forced += (a_shake * self.dt + v_rand) / self.dy
